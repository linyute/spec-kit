...
while [ $i -le $# ]; do
    ...
    case "$arg" in
        ...
        --number)
            ...
            ;;
        --help|-h)
            echo "用法：$0 [--json] [--short-name <名稱>] [--number N] <特徵描述>"
            echo ""
            echo "選項："
            echo "  --json              以 JSON 格式輸出"
            echo "  --short-name <名稱> 為分支提供自定義短名稱（2-4 個單字）"
...
if [ -z "$FEATURE_DESCRIPTION" ]; then
    echo "用法：$0 [--json] [--short-name <名稱>] [--number N] <特徵描述>" >&2
    exit 1
fi

# 透過搜尋現有的專案標記來尋找儲存庫根目錄的函式
find_repo_root() {
    local dir="$1"
    while [ "$dir" != "/" ]; do
...
if git rev-parse --show-toplevel >/dev/null 2>&1; then
    ...
    if [ -z "$REPO_ROOT" ]; then
        echo "錯誤：無法確定儲存庫根目錄。請從儲存庫內執行此腳本。" >&2
        exit 1
    fi
    HAS_GIT=false
fi
...
# 強制進行十進位解釋，以防止八進位轉換（例如：010 在八進位中是 8，但在十進位中應為 10）
...
if [ ${#BRANCH_NAME} -gt $MAX_BRANCH_LENGTH ]; then
    ...
    >&2 echo "[specify] 截斷為：$BRANCH_NAME (${#BRANCH_NAME} 位元組)"
fi
