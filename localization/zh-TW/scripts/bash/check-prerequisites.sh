...
EOF
            ...
            ;;
        *)
            echo "錯誤：未知選項 '$arg'。使用 --help 獲取用法資訊。" >&2
            exit 1
            ;;
    esac
done
...
# 如果是僅路徑模式，輸出路徑並退出（支援 JSON + 僅路徑組合）
if $PATHS_ONLY; then
    if $JSON_MODE; then
        # 最小 JSON 路徑負載（未執行驗證）
...
if [[ ! -d "$FEATURE_DIR" ]]; then
    echo "錯誤：找不到特徵目錄：$FEATURE_DIR" >&2
    echo "先執行 /speckit.specify 以建立特徵結構。" >&2
    exit 1
fi

if [[ ! -f "$IMPL_PLAN" ]]; then
    echo "錯誤：在 $FEATURE_DIR 中找不到 plan.md" >&2
    echo "先執行 /speckit.plan 以建立實作計畫。" >&2
    exit 1
fi
...
if $REQUIRE_TASKS && [[ ! -f "$TASKS" ]]; then
    ...
    echo "先執行 /speckit.tasks 以建立工作列表。" >&2
    exit 1
fi
